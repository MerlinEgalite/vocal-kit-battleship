    ...
    if count>=3:
        ...
        if last_hit[0]==True:
            ...
            if last_last_hit[0]==True: #Quand on enchaine 2 Hit on force le coomputer a tirer sur le même axe (Defaut : l'algo n'essaye d'aller que dans un sens)
                if Gauche==Case_last_last_hit and Playable_position(Droite[0],Droite[1],grid)==True:
                    shooting_list.append(None,Droite)
                    return tuple_to_position(Droite)
                if Droite==Case_last_last_hit and Playable_position(Gauche[0],Gauche[1],grid)==True:  
                    shooting_list.append(None,Gauche)
                    return tuple_to_position(Gauche)
                if Haut==Case_last_last_hit and Playable_position(Bas[0],Bas[1],grid)==True:
                    shooting_list.append(None,Bas)
                    return tuple_to_position(Bas)
                if Bas==Case_last_last_hit and Playable_position(Haut[0],Haut[1],grid)==True:
